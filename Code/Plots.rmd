...
## Descriptive statistics for the log returns
...
tickers <- c(first_half, second_half)

stats_tbl <- map_dfr(tickers, function(nm) {
  x <- df_clean[[nm]]
  x <- x[is.finite(x)]  # drops NA/NaN/Inf
  ...
  mutate(across(where(is.numeric), ~ round(.x, 6)))  # adjust digits if you want
...
writeLines(tab_tex, "desc_table1.tex")
...
```{r}
names(Z_xts)[names(Z_xts) == "X2CUREX.ST"] <- "2CUREX.ST"
Z_0 <- coredata(Z_xts)
...
# Use identical layout for fair comparison
layout_fixed <- qgraph(A1, DoNotPlot = TRUE)$layout
...
qgraph(A1, layout = layout_fixed,
       title = paste0("Days 1–250\nEdges: ", e1))

qgraph(A2, layout = layout_fixed,
       title = paste0("Days 251–500\nEdges: ", e2))
...
```{r}
#tvsfglasso is not on CRAN but can be installed from here:
#https://github.com/markkukuismin/tvsfglasso
library(igraph)
library(tvsfglasso)
...
#persistent issue due to some packages not liking column names starting with numbers
# Only rename if the column X2CUREX.ST exists
if ("X2CUREX.ST" %in% colnames(Z_0)) {
  colnames(Z_0)[colnames(Z_0) == "X2CUREX.ST"] <- "2CUREX.ST"
}
...
## Plot of Time-Varying GLASSO with sectors at t
...
# fallback for any sectors missing from the map
if (any(is.na(sector_cols))) {
  missing_secs <- sector_levels[is.na(sector_cols)]
  fallback_cols <- rainbow(length(missing_secs))
  names(fallback_cols) <- missing_secs
  sector_cols[is.na(sector_cols)] <- fallback_cols
}
...
## Bar for strength of correlation
